			     ...
			     struct wpa_state_machine *assoc_wpa_sm)
{
	...
	if (ap_sta_is_epp(sta) && hapd->conf->pmksa_caching_privacy &&
		...
		random_get_bytes(sta->anonce, NONCE_LEN);
				   ...
				   struct mld_link_info *link)
{
	...
	if (ap_sta_is_epp(sta)) {
		...
		if (assoc_sta) {
			...
			if (rsne && assoc_rsne &&
			    (rsne[1] != assoc_rsne[1] ||
			     os_memcmp(rsne, assoc_rsne, 2 + rsne[1]))) {
				...
				if (wpa_auth_ap_sta_support_pmkid_privacy(
					...
					case WLAN_AUTH_EPPKE:
						pmkid_next =
							assoc_sta->epp_pmkid_next;
						break;
					}
				}
#endif /* CONFIG_PMKSA_PRIVACY */

				wpa_insert_pmkid(p, &rsn_ie_len, pmkid_next,
						 true);
				p += rsn_ie_len;
			}
		}
...
rsnxe_done:
	...
	if (sta && sta->auth_alg == WLAN_AUTH_EPPKE &&
		...
		/* Per IEEE802.11bi/D4.0, 12.16.7.2 PMKSA caching privacy
			    ...
			    size_t len, int reassoc, int ok)
{
	...
	if (ok && status == WLAN_STATUS_SUCCESS && sta->epp_sta &&
		...
		if (!entry && is_ml) {
			...
			/* Search in link caches of each AP MLD link */
			for_each_mld_link(tmp_hapd, hapd) {
				t_pmksa = wpa_auth_get_pmksa_cache(tmp_hapd->wpa_auth,
								   false);
...
update_pmksa_entry:
		if (entry) {
			wpa_printf(MSG_DEBUG, "PMK Privacy on - update "
				   "PMKSA cache entry");
